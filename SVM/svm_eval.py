...
class SVM:
    ...
    def predict(self, X):
        w = self.w
        x = X
        prediction = np.dot(x, w) + self.b  # w.x + b
        return np.sign(prediction)

    def save(self, path: str):
        params = list(self.w[0])
        params.insert(0, self.C)
...
if __name__ == '__main__':
    ...
    for dataframe in dataframes:
        ...
        w, b, losses = svm_own.fit(samples_train, labels_train, batch_size=200, learning_rate=0.01, epochs=2000)
        path = f'.\\models\\{dataframe}_model'
        svm_own.save(path=path)
